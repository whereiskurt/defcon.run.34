...
echo "Next steps:"
echo ""
echo "  1. Set the key ID in env.local.sh:"
echo "     export TF_VAR_SOPS_KMS_KEY_ID=\"${KEY_ID}\""
echo ""
echo "  2. Populate secrets from the template:"
echo "     cp infra/terraform/live/site/.secrets.sops.json.template \\"
echo "        infra/terraform/live/site/.secrets.json"
echo "     # Edit .secrets.json — replace all CHANGEME values"
echo ""
echo "  3. Encrypt with SOPS:"
echo "     cd infra/terraform/live/site"
echo "     sops --encrypt .secrets.json > .secrets.sops.json"
echo "     rm .secrets.json"
echo ""
echo "  4. Run Terragrunt:"
echo "     cd infra/terraform/live/site && terragrunt plan --all"
