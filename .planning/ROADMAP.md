...
- [~] 73-03-PLAN.md — Wave 2 (PARTIAL — ✅ the upstream pre-gate is now CLEARED: meshtk **#36 MERGED** 2026-08-01T06:44Z as `f8ac7b3`, so `resolve_meshtk`'s fresh clone of meshtk `main` already carries the limiter. Monorepo **#1158 MERGED** as `5ac49845` with the 6 Terraform files (metric filter, alarm, site.hcl threshold, `MESHTK_LLM_CALLS_PER_HOUR` on the ghosts container); its `.planning/` half had landed separately in `2cc8c574`. Both merge gates are therefore CLEARED and only the release remains, STOPPED at the blocking human approval gate): buildpub release (all four run.mqtt components) → `deploy.yml` → read-only live probe
