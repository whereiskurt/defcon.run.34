...
- [x] 73-03-PLAN.md — Wave 2 (**SHIPPED AND VERIFIED LIVE 2026-08-01**): meshtk #36 merged `f8ac7b3` 06:44Z → monorepo #1158 merged `5ac49845` → buildpub 12:29Z cloned meshtk fresh at 12:30:10Z (after the merge, so v0.0.85 carries the limiter) → `#1159` bumped meshtk v0.0.84→**v0.0.85** → `deploy.yml` PR:skip 17:15Z → SCOPED `terragrunt-apply modules=admin-reports` 17:24Z. Read-only probe against AWS: task def **127** RUNNING+HEALTHY on `dc34-run-mqtt-meshtk:v0.0.85`, `MESHTK_LLM_CALLS_PER_HOUR=60` on the ghosts container, `MESHTK_GUARDRAIL_FAILMODE=closed` still intact, alarm `dcr-mqtt-llm-rate-limit` OK @ threshold 20 → tripwire SNS, metric filter `dcr-mqtt-llm-rate-limits` present. ⚠️ `deploy.yml` does NOT apply `admin-reports`, so the limiter was enforced with no alarm for the ~9 min between 17:15Z and 17:24Z — a scoped apply must always follow a release that adds observability. Hardware UAT deferred to `.planning/todos/pending/2026-08-01-llm-rate-limit-hardware-uat.md` (human-only), following the Phase 72 precedent of closing on proven-live rather than on UAT
