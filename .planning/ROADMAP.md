...
**Depends on:** Phase 49 (`Accomplishment` entity + `createAccomplishment`).
**Requirements:** LDBR-05 (`run.gpx`: decimate polyline on activation, store `GpxFile.polyline`; non-fatal on failure; reuse the haversine/track-walk from `seed-local-routes.ts`), LDBR-06 (run.human `POST /api/internal/accomplishment`: `AUTH_INTERNAL_SECRET` gate, OIDC-sub→adapter-id bridge, idempotent on `gpxFileId`, drops with a log when no `RunUser` exists for the sub)
**Success Criteria** (what must be TRUE):

  1. Activating a GPX file stores a decimated `polyline` on its `GpxFile` row and produces exactly one `gpx` `Accomplishment` for the owning run.human `RunUser`, raising `activityScore` + `activityCounts.gpx`.
  ...
  4. A GPX-notify failure leaves the upload/save successful (best-effort, never blocks the user).
