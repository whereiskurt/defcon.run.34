...
## Current Milestone: v1.9 CMS-Driven UI Copy Catalog

**Goal:** Make static UI copy across all five apps editable from Strapi without a deploy — via a single global copy catalog, a cached copy toolkit with an S3 fallback, and a custom admin page.

**Target features:**
- `ui-string` Strapi content type (`key·locale·value·namespace·notes`) + `(key,locale)` uniqueness + read-only API-token permission
- Copy toolkit: `loadCopy` calls the Strapi API cached in the Next.js Data Cache (`revalidate:N`, eventual ~15 min over the existing master/worker + Litestream topology); returns one merged map; `t()` / `CopyProvider` / `useCopy` so copy works in server render AND client modals/toasts
- Cached fallback: S3 copy export (regenerated by a Strapi lifecycle hook) + optional committed snapshot floor; hard requirement that the fallback path is as fast/cached as the live path
- Custom three-column admin plugin (`label·locale·value`, namespace filter, bulk upsert)
- Cross-cutting string migration across run.human/auth/gpx/bib/flash — proof surface first: bib donate/sponsor
- Multi-lingual-ready (`locale` column; only `default` populated now)

**Key context:** Words-only scope (each app keeps its own React header, reads labels by shared key — no shared component rewrite). No Redis, no revalidation webhook — eventual consistency rides the existing master/worker + Litestream topology. Reuses the existing S3→CloudFront media path for the export. Full design spec: `docs/superpowers/specs/2026-07-05-cms-copy-catalog-design.md`.
