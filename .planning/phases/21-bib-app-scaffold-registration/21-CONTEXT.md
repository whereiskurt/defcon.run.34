...
- **Provided reference + starter template (use these):** `assets/bib-reference-dc34.png` is the user-approved DC34 bib layout, and `assets/bib-template.svg` is a ready SVG implementation of it — DEFCON banner (the "O" is the **DEF CON smiley**, a round smiley face over crossbones — NOT a pirate skull), a black number box with `id="bib-number"`, the name band `id="bib-name"` **directly under the number inside the box** (empty text hides it), corner smiley badges, pin holes, perforation, and two tear-off stubs. The bib component should render from this SVG (fixed `viewBox`) and set font-size on `#bib-number` / `#bib-name` for auto-shrink (short → large, long → smaller; never wrap/truncate); when no name is present it may re-center `#bib-number` in the full box. Tokens `{{BIB_NUMBER}}` / `{{NAME_ON_BIB}}` mark the dynamic text.
