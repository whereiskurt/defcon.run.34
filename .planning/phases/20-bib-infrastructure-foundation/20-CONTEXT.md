# Phase 20: Bib Infrastructure Foundation - Context
...
### Persistence
- Reuse the shared `run-human-electro` DynamoDB table via a new `Bib` ElectroDB entity (Phase 21). No new table is created in Phase 20.
...
### Secrets (SSM placeholders only in Phase 20)
...
- Real/live values are populated out-of-band; dev uses Stripe test-mode + PayPal sandbox creds. Phase 20 only ensures the parameters exist and are referenced by service.hcl `secrets`
...
- The auth server must register a `bib` OIDC client and grant the `bib` service claim; that registration is a Phase 21 dependency, but the SSM `/bib/client_id` + `/bib/client_secret` slots are created here
...
- Stripe secrets are referenced in service.hcl now (so Phase 22 needs no infra change) but resolve to placeholder values until populated
...
- Actual container images and Next.js code → Phase 21
- Stripe wiring and real secret values → Phase 22
- build.sh/deploy.sh/CI integration → Phase 23

</deferred>

---

*Phase: 20-bib-infrastructure-foundation*
