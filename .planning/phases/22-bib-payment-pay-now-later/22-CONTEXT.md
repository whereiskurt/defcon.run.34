# Phase 22: Bib Payments (Cash + Stripe + PayPal/Venmo, crypto-ready) - Context

**Gathered:** 2026-06-30
**Status:** Ready for planning (plan in detail when Phase 21 completes)
...
- Env/secret contract: Stripe (`STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`) and **PayPal (`PAYPAL_CLIENT_ID`, `PAYPAL_CLIENT_SECRET`, `PAYPAL_WEBHOOK_ID`)** SSM slots are created in Phase 20; each future provider adds its own keys under `/{site}/secrets/{region}/bib/<provider>_*` (one-line infra add, no schema change). Browser-needed client ids are `NEXT_PUBLIC_*`.
...
- **Stripe test mode:** use `sk_test_…` / `pk_test_…` keys (in the Phase-20 SSM placeholders for a staging deploy, or `.env.local` for dev). No real charges.
...
- Suggested plan split: 22-01 seam+registry+fake+webhook route+state machine+UI+cash; 22-02 Stripe provider; 22-03 PayPal/Venmo provider — so each provider is an isolated, independently testable unit

</specifics>

<code_context>
## Existing Code Insights
- Bib entity + API routes land in Phase 21 — payment fields (`paymentStatus`, `amount`, `currency`, `paymentProvider`) defined on the entity there
- Stripe + PayPal secrets wired into service.hcl + SSM in Phase 20 (no infra change needed here)
- Auth: protect payment routes with the run.gpx `auth` export + `bib` claim (Phase 21)
...
---
*Phase: 22-bib-payment-pay-now-later*
