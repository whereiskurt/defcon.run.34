# Phase 20: Bib App Scaffold + Registration - Context

**Gathered:** 2026-06-30
**Status:** Ready for planning (plan in detail when Phase 19 completes)

<domain>
## Phase Boundary

Stand up the bib.defcon.run Next.js webapp (mirroring `apps/run.flash/webapp/`) and the registration data path: a `Bib` ElectroDB entity on the shared electro table, a registration form (name on bib), create/fetch API routes, and the OIDC `bib`-claim auth gate. Plus the nginx reverse-proxy container. No payment logic (Phase 21), no release pipeline wiring (Phase 22).
...
- Service claim string is `bib`; OIDC secrets resolve from `/{site}/secrets/{region}/bib/client_id|client_secret` (created Phase 19)
...
- Required run.auth changes for bib: add a `bib` client to `oidc.ts`/`index.ts` (redirect_uris = `https://bib.{{SITE_DOMAIN}}/{{REGION_LABEL}}/api/auth/callback/...` mirroring gpxStudio), add `OIDC_BIB_CLIENT_ID`/`OIDC_BIB_SECRET` env + matching SSM, and ensure the `bib` string is grantable as a service claim. run.auth must be redeployed for the client to take effect (fold into the Phase 22 release, or an interim run.auth deploy).
...
### Bib entity (shared electro table)
...
- Attributes: `bibId`, owner OIDC `sub`, `nameOnBib`, `bibNumber` (assigned display number, see visual below), `size` (optional), `paymentStatus` (`unpaid` | `pending_payment` | `pay_on_site` | `paid`), `amount`, `currency`, `paymentProvider` (`none` | `cash` | `stripe` | `paypal` | `coinbase` | …; provider-agnostic per Phase 21), `createdAt`, `updatedAt`
- Index to fetch a user's registration by `sub` (avoid colliding with existing run-user/checkin GSIs — follow the CheckIn gsi2/gsi3 precedent)
- One active registration per user (idempotent create)
...
- Submit persists an `unpaid` registration, then routes to the confirmation/payment step (payment is Phase 21)
...
- `bib` auth client must be registered on the auth server (dependency surfaced to operator); SSM client_id/secret slots already exist from Phase 19
...
<deferred>
## Deferred Ideas
- Stripe / payment → Phase 21
- build/deploy/CI wiring → Phase 22

</deferred>

---
*Phase: 20-bib-app-scaffold-registration*
