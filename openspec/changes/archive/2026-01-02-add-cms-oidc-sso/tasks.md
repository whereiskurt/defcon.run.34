...
- [x] 1.1 Update `apps/run.cms/app/config/admin.ts`:
...
- [x] 1.2 Update `apps/run.cms/app/package.json`:
  - Remove `passport-openidconnect` from dependencies

## 2. Install and Configure Community Plugin

- [x] 2.1 Install strapi-plugin-sso:
  ```bash
  cd apps/run.cms/app
  npm install strapi-plugin-sso@^1.0.7
  ```

- [x] 2.2 Update `apps/run.cms/app/config/plugins.ts` to add plugin config:
  ```typescript
  'strapi-plugin-sso': {
    enabled: true,
    config: {
...
- [x] 3.1 Create directory `apps/run.cms/app/src/extensions/strapi-plugin-sso/`

- [x] 3.2 Create `apps/run.cms/app/src/extensions/strapi-plugin-sso/strapi-server.ts`:
...
- [x] 4.1 Update `apps/run.auth/webapp/src/config/oidc.ts`:
...
- [x] 4.2 Rebuild and deploy auth service

## 5. Update Infrastructure

- [x] 5.1 Update `infra/terraform/live/site/services/cms/service.hcl`:
...
- [x] 6.1 Rebuild CMS app Docker image with new plugin and extension
- [x] 6.2 Deploy infrastructure changes (ALB route)
- [x] 6.3 Deploy CMS to us-east-1

## 7. Validation

- [x] 7.1 Navigate to `https://cms.defcon.run/use1/strapi-plugin-sso/oidc`
- [x] 7.2 Verify redirect to auth.defcon.run for authentication
- [x] 7.3 Test login with user who has "cms" in services claim - should succeed
- [x] 7.4 Test login with user without "cms" in services claim - should see access denied error
- [x] 7.5 Verify new SSO user is auto-created with correct name and email
- [x] 7.6 Verify existing SSO user is recognized on subsequent logins
- [x] 7.7 Verify local admin login still works at `/use1/admin` (fallback access)
