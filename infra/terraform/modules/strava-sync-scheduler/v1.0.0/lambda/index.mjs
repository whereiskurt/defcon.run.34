// strava-sync-scheduler invoker Lambda (v1.7 Phase 33).
//
// EventBridge Scheduler fires this on a cron; it reads the shared internal secret from
// SSM and POSTs the run.gpx internal sync endpoint (which does the actual Strava pull).
// Keeping the schedule → HTTP hop in a tiny Lambda avoids exposing the worker publicly
// and matches the repo's Lambda-per-concern convention (see bib-reconcile-lambda).
//
