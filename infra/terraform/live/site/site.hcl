  ...
  secrets = {
    ...
    definitions = {
      ...
      mqtt = {
        ...
        # encrypted .secrets.sops.json (generated by the 72-08 rotation) and reaches
        # the task only via SSM valueFrom; never git, a plan, or a log. Empty is
        # tolerated: meshtk then sends the award line without a link.
        keys = ["meshtk-proxy-password", "meshobserv-password", "ghosts-password", "max-connections", "s3-log-interval", "channel-psk", "ghost-start-delay", "ghost-key-secret", "flag-challenges", "ricky-fallback-url"]
      }
    }
  }
