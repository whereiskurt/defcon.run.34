...
export REPO_NAME="dc34-run-human-app"
export WEBAPP_ORIGIN="run.defcon.run"
export WEBAPP_PREFIX=${WEBAPP_PREFIX:-"use1/assets"}
export REGION_SHORT=${REGION_SHORT:-"use1"}
export WEBAPP_ORIGIN_BUCKET=$(aws ssm get-parameter --name "/dc34/cloudfront-assets/${REGION_SHORT}/run/bucket_name" --region "${AWS_REGION}" --query "Parameter.Value" --output text)
