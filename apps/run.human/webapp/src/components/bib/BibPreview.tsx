...
/**
 * Stamp wording, inlined.
 *
 * PORT NOTE (run.human): the run.bib original resolved these five words through
 * its CMS copy catalog. They are lettering baked into the bib ARTWORK, not
 * run.human prose, so the port carries them as constants instead of adding
 * bib-scoped keys to run.human's catalog (which has its own guard test) or
 * making a rendered SVG depend on CMS availability. Keep in sync with
 * run.bib's `bib.status.stamp*` keys if the artwork wording ever changes.
 */
const STAMP = {
  stampUnsaved: "UNSAVED",
  stampSaved: "SAVED",
  stampDraft: "DRAFT",
  stampPaid: "PAID!",
  stampThankYou: "THANK YOU!",
} as const;
