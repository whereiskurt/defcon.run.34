  ...
  const autoPair = useCallback(async (qr: RunnerQr) => {
    ...
    try {
      ...
      if (res.ok && data.code === 'bib_pickup') {
        ...
        // Operator just primed a bib. Covers BOTH a fresh pair and
        // code:'bib_ready' (the pair was already spent today) — priming worked
        // either way, so this must never render as a duplicate or an error.
        // The server sends bibStatus only for the ONE scan that primes, so a
        // re-scan of the same runner flashes the ordinary PAIRED below.
        setTally((t) => ({ ...t, ok: t.ok + 1 }));
        if (soundRef.current) playBeep(audioCtxRef, 'ok');
        flashNow('ok', `BIB READY · ${data.ownerName ?? 'runner'}`);
      } else if (res.ok) {
        setTally((t) => ({ ...t, ok: t.ok + 1 }));
        if (soundRef.current) playBeep(audioCtxRef, 'ok');
