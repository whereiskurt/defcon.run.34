        ...
        return new Promise((resolve) => {
            ...
            this._subscription = liveQuery(() => db.fileids.toArray()).subscribe((dbFileIds) => {
                ...
                if (newFiles.length > 0 || deletedFiles.length > 0) {
                    ...
                    // Update the file order (ensure it's always an array)
                    const currentFileOrder = get(settings.fileOrder);
                    let fileOrder = Array.isArray(currentFileOrder)
                        ? currentFileOrder.filter((id) => !deletedFiles.includes(id))
                        : [];
                    newFiles.forEach((id) => {
