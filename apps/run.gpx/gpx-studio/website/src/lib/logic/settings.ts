...
export class Setting<V> {
    ...
    connectToDatabase(db: Database) {
        ...
        this._subscription = liveQuery(() => db.settings.get(this._key)).subscribe((value) => {
            // Only update if we get a valid value from the database
            // If undefined, keep the current value (or initial if never set)
            if (value !== undefined) {
...
export class SettingInitOnFirstRead<V> {
    ...
    connectToDatabase(db: Database) {
        ...
        this._subscription = liveQuery(() => db.settings.get(this._key)).subscribe((value) => {
            if (value === undefined) {
                // On first read, use initial value; on subsequent reads, keep current value
                if (first) {
                    this._value.set(this._initial);
                }
                // Don't set to undefined on subsequent reads - keep current value
            } else {
                this._value.set(value);
            }
            first = false;
...
export const settings = {
    ...
    lastSaveFolder: new Setting<string>('lastSaveFolder', 'ROOT'),
