/**
 * Check-in pin icon catalog — RENDERING MIRROR (v1.8 Phase 4).
 *
 * The studio is a separate Vite build and cannot import run.human's
 * `src/lib/pin-icons.ts`, so the catalog is mirrored here (same pattern as
 * dc34-palette). Keep ids and SVG output in sync with that file — the map
 * renders whatever icon id the public API serves. Gating (`requiredService`)
 * is enforced server-side and does not exist here.
 */
