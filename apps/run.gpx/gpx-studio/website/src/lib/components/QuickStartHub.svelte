...
<style>
    /* Mirrors Menu.svelte's .add-run-glow so every "add a run" surface reads the
       same: bright green, softly pulsing, still legible with reduced motion. */
    .add-run-glow {
    ...
    /* Strava brand orange, so orange consistently means Strava and green
