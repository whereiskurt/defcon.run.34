<script lang="ts">
    ...
    function createLayer() {
        ...
        const validUrls = tileUrls.map((url) => decodeURI(url.trim())).filter((url) => url.length > 0);
        if (validUrls.length === 0) {
            // Don't create a layer with no valid URLs
            return;
        }
        ...
        let is512 = validUrls.some((url) => url.includes('512'));
