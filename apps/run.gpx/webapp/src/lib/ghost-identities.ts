/**
 * Ghost slug → dossier for the reveal popup. Each ghost node in the mesh is a
 * "flatlined" construct of a hacker-culture figure (see the persona prompts in
 * apps/run.mqtt/meshtk/meshtk.dc34.yaml). The popup shows who the ghost echoes.
 *
 * Blurbs are strictly biographical — they must NEVER hint at the covert CTF
 * flag phrases or secret codes baked into those persona prompts.
 */
...
/** Ghost slug → display name. Unknown slug → title-cased fallback. */
