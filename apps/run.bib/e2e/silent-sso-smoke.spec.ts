...
/**
 * Silent-SSO warm-session SMOKE for run.bib.
 *
 * Mirrors run.gpx/e2e/silent-sso.spec.ts trimmed to the single warm invariant:
 * for a warm run.auth session the bib app reaches its authenticated view with
 * the app session cookie (`sess_bib`) established via the hidden prompt=none
...
// Defaults to local bib dev (:3004); override BASE_URL for prod.
...
// Public entry route (default region root; bib auth-gates every route to /signin).
const ENTRY_PATH = process.env.SSO_ENTRY_PATH || `${REGION_PREFIX}/`;
// App session cookie the silent iframe establishes on success.
const APP_SESSION_COOKIE = process.env.SSO_APP_COOKIE || 'sess_bib';
...
test.describe('Silent SSO (run.bib smoke) — warm', () => {
  test.beforeEach(async () => {
    test.skip(
      !hasAuthCookieJar(),
      `[SKIP] No warm run.auth cookie jar at ${getAuthCookieJarPath()} — the bib warm smoke cannot run. ` +
        `Acquire a session (run.auth e2e) first. A green --list does NOT prove the invariant.`,
    );
    test.skip(
      !(await stackReachable()),
      `[SKIP] bib stack not reachable (app ${BASE_URL} + IdP ${AUTH_SERVICE_URL}) — start the bib dev server + run.auth to run the warm smoke.`,
    );
  });

  test('warm session: authed view renders, no /login render, no loop, sess_bib set via iframe [warm]', async ({
    page,
    context,
  }) => {
