...
const redirectProxyUrl = process.env.BIB_PUBLIC_URL
  ? `${process.env.BIB_PUBLIC_URL}/api/auth`
  : (isDev
    ? `http://localhost:${LOCAL_BIB_PORT}/api/auth`
    : `https://bib.${siteDomain}/${region}/api/auth`);
...
export async function fetchFreshClaims(userId: string): Promise<FreshClaimsResult> {
  ...
  try {
    ...
    if (!response.ok) {
      console.error(
        "[run.bib] Failed to fetch claims:",
        response.status,
        "URL:",
        `${validateUrl}/${userId}`
      );
    ...
    console.error("[run.bib] Error fetching fresh claims:", error);
    return null;
  }
}
    ...
    async jwt({ token, account, profile, trigger }) {
      ...
      const validateAndUpdateClaims = async (
        ...
        if (freshClaims.lockedOut) {
          console.log(
            `[run.bib] User ${userId} is locked out, invalidating session`
          );
          return false;
        }
        ...
        if (freshClaims.sessionVersion > tokenVersion) {
          console.log(
            `[run.bib] Session version mismatch for ${userId}: token=${tokenVersion}, server=${freshClaims.sessionVersion}`
          );
          return false;
        }
