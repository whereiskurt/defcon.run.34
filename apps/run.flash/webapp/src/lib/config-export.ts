...
export function toReadableText(p: DeviceConfigPayload): string {
  ...
  return [
    ...
    `  Frequency slot: ${p.radio.channelNum}`,
    `  Hop limit: ${p.radio.hopLimit}`,
    ``,
    `== Device (app: Settings → Radio Configuration → Device) ==`,
    `  Rebroadcast mode: ${p.device.rebroadcastMode}`,
    ``,
...
export function toCliScript(p: DeviceConfigPayload): string {
  ...
  const lines = [
    ...
    ``,
    `meshtastic --set lora.region ${p.radio.region} --set lora.modem_preset ${p.radio.modemPreset} --set lora.channel_num ${p.radio.channelNum} --set lora.hop_limit ${p.radio.hopLimit}`,
    `meshtastic --set device.rebroadcast_mode ${p.device.rebroadcastMode}`,
