...
// Load one URL in a fresh context and sum the transferred bytes of every response.
...
try {
  for (const s of data.services) {
    ...
    try {
      const { bytes, reqs } = await measure(browser, url);
      ...
      if (kb < 20) {
        delete s.size_kb; delete s.req_count; delete s.measured_at;
        console.log(`${kb} KB · ${reqs} reqs — too small (gated/blocked?), left unmeasured`);
      } else {
        s.size_kb = kb;
        s.req_count = reqs;
        s.measured_at = now;
        console.log(`${s.size_kb} KB · ${reqs} reqs`);
      }
    } catch (e) {
